 ...
 */
export function parseOrderFromResponse(response: string, userMessage?: string, conversationHistory?: Array<{role: string, content: string}>): ParsedOrder {
  ...
  const patterns = {
    ...
    materiau: /(?:matériau\s*:?\s*)([^\n]+)/i,
    quantite: /(?:quantité.*?:?\s*)([\d,.\s]+)/i,
    unite: /(?:quantité.*?:?\s*[\d,.\s]+\s*)(m³|kg|m²|tonnes?|sacs?|palettes?|m|cm|litres?|l)/i,
    date_besoin: /(?:besoin pour.*?:?\s*|pour.*?:?\s*)((?:\d{1,2}\/\d{1,2}\/\d{4})|(?:\d{4}-\d{2}-\d{2}))/i,
    heure_besoin: /(?:besoin pour.*?(?:\d{1,2}\/\d{1,2}\/\d{4}||\d{4}-\d{2}-\d{2}).*?|pour.*?(?:\d{1,2}\/\d{1,2}\/\d{4}||\d{4}-\d{2}-\d{2}).*?)(\d{1,2}:\d{2})/i,
  };
  ...
  try {
    ...
    // Si on a l'historique de conversation, l'utiliser pour extraire plus d'informations
    let fullContext = cleanResponse;
    if (conversationHistory && conversationHistory.length > 0) {
      // Récupérer les 10 derniers messages pour le contexte
      const recentMessages = conversationHistory.slice(-10);
      fullContext = recentMessages.map(msg => msg.content).join(' ') + ' ' + cleanResponse;
    }

    // Essayer d'extraire avec les patterns principaux (d'abord dans la réponse, puis dans le contexte complet)
    const chantierMatch = fullContext.match(patterns.chantier);
    if (chantierMatch && chantierMatch[1]) {
      result.chantier = chantierMatch[1].trim();
    }

    const materiauMatch = fullContext.match(patterns.materiau);
    if (materiauMatch && materiauMatch[1]) {
      result.materiau = materiauMatch[1].replace(/^-\s*/, '').trim();
    }

    const quantiteMatch = fullContext.match(patterns.quantite);
    if (quantiteMatch && quantiteMatch[1]) {
      result.quantite = quantiteMatch[1].trim();
    }

    const uniteMatch = fullContext.match(patterns.unite);
    if (uniteMatch && uniteMatch[1]) {
      result.unite = uniteMatch[1].toLowerCase();
    }

    const dateMatch = fullContext.match(patterns.date_besoin);
    if (dateMatch && dateMatch[1]) {
      result.date_besoin = dateMatch[1];
    }

    const heureMatch = fullContext.match(patterns.heure_besoin);
    if (heureMatch && heureMatch[1]) {
      result.heure_besoin = heureMatch[1];
    }

    // Si les patterns principaux n'ont pas fonctionné, essayer les alternatifs sur le contexte complet
    if (!result.chantier) {
      const altChantierMatch = fullContext.match(alternativePatterns.chantier);
    ...
    if (!result.materiau) {
      const altMateriauMatch = fullContext.match(alternativePatterns.materiau);
    ...
    if (!result.quantite || !result.unite) {
      const altQuantiteMatch = fullContext.match(alternativePatterns.quantite);
    ...
    if (!result.date_besoin) {
      const altDateMatch = fullContext.match(alternativePatterns.date);
    ...
    if (!result.heure_besoin) {
      const altHeureMatch = fullContext.match(alternativePatterns.heure);
